...
$(SO_NAME): $(SOURCES) $(HEADERS)
	...
	$(NVCC) -c -Xcompiler "-fPIC -Wall" $(OPTIMISE) $(DEBUG) $(GPU_GENCODE) $(INCLUDE) -o $(OBJ_DIR)/dedisp.o $(SRC_DIR)/dedisp.cu
...
$(PTX_NAME): $(SOURCES) $(LIB_DIR)/libdedisp.so $(HEADERS)
	...
	$(NVCC) -ptx -Xcompiler "-fPIC -Wall" $(OPTIMISE) $(DEBUG) $(GPU_GENCODE) $(INCLUDE) -o $(PTX_NAME) $(SRC_DIR)/dedisp.cu
